...
export function MonitoringPage({ token }: Props) {
  const monitoringQuery = useMonitoring(token)
  const status = monitoringQuery.data?.status ?? null
  const metrics = monitoringQuery.data?.metrics ?? {}
  const updatedAt = monitoringQuery.data?.updatedAt ? new Date(monitoringQuery.data.updatedAt) : null
  const error = monitoringQuery.error instanceof Error ? monitoringQuery.error.message : null
  ...
  return (
    <section className="space-y-4">
      <PageHeader
        title="Monitoring"
        description="Watch service counters, TLS certificate health, and runtime capacity signals with a live refresh loop."
        actions={
          <>
            <Badge variant="outline">5s refresh</Badge>
            <Button variant="outline" onClick={() => void monitoringQuery.refetch()} disabled={monitoringQuery.isFetching}>
              <RefreshCw className={`mr-2 h-4 w-4 ${monitoringQuery.isFetching ? "animate-spin motion-reduce:animate-none" : ""}`} />
        ...
        <CardContent className="space-y-4">
          ...
          {monitoringQuery.isLoading ? (
            ...
            <EmptyState
              ...
              description="The server did not return any metrics for the dashboard yet."
              icon={Activity}
            />
          ) : (
            <>
